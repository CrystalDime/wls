...
/// Remove `.pre` when tagging a new wls release and add it back on the next development cycle.
const wls_version = std.SemanticVersion{ .major = 0, .minor = 13, .patch = 0, .pre = "dev" };

/// Specify the minimum Zig version that is required to compile and test wls:
/// rework std.Progress
///
/// Must match the `minimum_zig_version` in `build.zig.zon`.
const minimum_zig_version = "0.13.0-dev.386+3964b2a31";
...
pub fn build(b: *Build) !void {
    ...
    const resolved_wls_version = getVersion(b);
    const resolved_wls_version_string = b.fmt("{}", .{resolved_wls_version});

    const build_options = b.addOptions();
    build_options.step.name = "WLS build options";
    const build_options_module = build_options.createModule();
    build_options.addOption(std.SemanticVersion, "version", resolved_wls_version);
    build_options.addOption([]const u8, "version_string", resolved_wls_version_string);
    build_options.addOption([]const u8, "minimum_runtime_zig_version_string", minimum_runtime_zig_version);

    const exe_options = b.addOptions();
    exe_options.step.name = "WLS exe options";
    ...
    const global_cache_path = try b.cache_root.join(b.allocator, &.{"wls"});
    ...
    test_options.step.name = "WLS test options";
    ...
    const resolved_data_version = data_version orelse if (wls_version.pre == null and wls_version.build == null) wls_version else "master";
    ...
    const wls_module = b.addModule("wls", .{
    ...
    for (targets) |target_query| {
        ...
        exe.root_module.addImport("wls", wls_module);
    ...
    tests.root_module.addImport("wls", wls_module);
...
fn getVersion(b: *Build) std.SemanticVersion {
    if (wls_version.pre == null and wls_version.build == null) return wls_version;
    ...
    switch (std.mem.count(u8, git_describe, "-")) {
        0 => {
            ...
            std.debug.assert(std.mem.eql(u8, git_describe, b.fmt("{}", .{wls_version}))); // tagged release must match version string
            return wls_version;
        },
        2 => {
            ...
            std.debug.assert(wls_version.order(ancestor_ver) == .gt); // wls version must be greater than its previous version
            std.debug.assert(std.mem.startsWith(u8, commit_id, "g")); // commit hash is prefixed with a 'g'

            return std.SemanticVersion{
                .major = wls_version.major,
                .minor = wls_version.minor,
                .patch = wls_version.patch,
...
const Build = blk: {
    ...
    std.debug.assert(wls_version.pre == null or std.mem.eql(u8, wls_version.pre.?, "dev"));
    std.debug.assert(wls_version.build == null);
    const wls_version_is_tagged = wls_version.pre == null and wls_version.build == null;

    if (min_runtime_zig.order(min_zig) == .gt) {
        const message = std.fmt.comptimePrint(
            \\A Zig version that is able to build WLS must be compatible with WLS at runtime.
            \\
    ...
    // check that the wls version and minimum build version make sense
    if (wls_version_is_tagged) {
        if (wls_version.order(min_zig) != .eq) {
            const message = std.fmt.comptimePrint(
                \\A tagged release of wls should have the same tagged release of Zig as the minimum build requirement:
                \\          wls version: {[current_version]}
            ...
            , .{ .current_version = wls_version, .minimum_version = min_zig });
            @compileError(message);
        }
    } else {
        const min_zig_simple = std.SemanticVersion{ .major = min_zig.major, .minor = min_zig.minor, .patch = 0 };
        const wls_version_simple = std.SemanticVersion{ .major = wls_version.major, .minor = wls_version.minor, .patch = 0 };
        const min_zig_is_tagged = min_zig.build == null and min_zig.pre == null;
        if (!min_zig_is_tagged and wls_version_simple.order(min_zig_simple) != .eq) {
            const message = std.fmt.comptimePrint(
                \\A development build of wls should have a tagged release of Zig as the minimum build requirement or
                ...
                \\
                \\This is a developer error.
            , .{ wls_version.major, wls_version.minor, min_zig });
            @compileError(message);
        }
    }
    ...
    if (switch (builtin.zig_version.order(min_zig)) {
        .lt => true,
        .eq => false,
        // a tagged release of wls must be build with a tagged release of Zig that has the same major and minor version.
        .gt => wls_version_is_tagged and (min_zig_simple.order(current_zig_simple) != .eq),
    }) {
        const message = std.fmt.comptimePrint(
        ...
        ++ if (is_min_zig_tagged_release)
            std.fmt.comptimePrint(
                ...
                \\
                \\Tagged releases of wls are also available.
                \\  -> https://github.com/zigtools/wls/releases
                \\  -> https://github.com/zigtools/wls/releases/tag/{[minimum_version_simple]} (may not exist yet)
            , .{
                .minimum_version = min_zig,
                .minimum_version_simple = min_zig_simple,
            })
        else if (is_current_zig_tagged_release)
            \\Please download or compile a tagged release of wls.
            \\  -> https://github.com/zigtools/wls/releases
            \\  -> https://github.com/zigtools/wls/releases/tag/{[current_version]} (may not exist yet)
        else
            ...
            \\  - Compile an older version of wls that is compatible with your Zig version
        , .{
            .current_version = builtin.zig_version,
            .minimum_version = min_zig,
            .required_zig_version_note = if (!wls_version_is_tagged) "(or greater)" else "",
        });
        @compileError(message);
    }
    break :blk std.Build;
