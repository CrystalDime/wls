...
const logger = std.log.scoped(.wls_main);
...
fn logFn(
    ...
    stderr.print("{s:<5}: ({s:^6}): ", .{ level_txt, if (comptime std.mem.startsWith(u8, scope_txt, "wls_")) scope_txt[4..] else scope_txt }) catch return;
...
fn parseArgs(allocator: std.mem.Allocator) !ParseArgsResult {
    ...
    const help_message: []const u8 = comptime help_message: {
        ...
        var cmd_infos: InfoMap = InfoMap.init(.{
            ...
            .@"minimum-runtime-version" = "Prints the minimum Zig version that is need to run wls.",
    ...
    const wls_exe_path = args_it.next() orelse @panic("");
    result.wls_exe_path = try allocator.dupe(u8, wls_exe_path);
    ...
    if (specified.get(.version)) {
        try stdout.writeAll(wls.build_options.version_string ++ "\n");
        return result;
    }
    if (specified.get(.@"minimum-runtime-version")) {
        try stdout.writeAll(wls.build_options.minimum_runtime_zig_version_string ++ "\n");
        return result;
    }
    ...
    if (specified.get(.@"show-config-path")) {
        var config_result = if (result.config_path) |config_path|
            try wls.configuration.loadFromFile(allocator, config_path)
        else
            try wls.configuration.load(allocator);
        ...
        switch (config_result) {
            ...
            .not_found => logger.info("No config file wls.json found.", .{}),
        }

        logger.info("A path to the local configuration folder will be printed instead.", .{});
        const local_config_path = wls.configuration.getLocalConfigPath(allocator) catch null orelse {
            logger.err("failed to find local wls.json file", .{});
            std.process.exit(1);
        };
...
pub fn main() !void {
    ...
    var failing_allocator_state = if (exe_options.enable_failing_allocator) wls.debug.FailingAllocator.init(inner_allocator, exe_options.enable_failing_allocator_likelihood) else void{};
    ...
    logger.info("Starting wls {s} @ '{s}'", .{ wls.build_options.version_string, result.wls_exe_path });

    var transport = wls.Transport.init(
        std.io.getStdIn().reader(),
        std.io.getStdOut().writer(),
    );
    transport.message_tracing = result.message_tracing_enabled;

    const server = try wls.Server.create(allocator);
